...
class DocLinkTests(unittest.TestCase):
    ...
    def test_repository_escape_fails(self) -> None:
        source = self.write("docs/guide.md", "[outside](../../outside.md)\n")
        findings = MODULE.check_file(self.root, source)
        self.assertEqual("link escapes repository", findings[0].reason)
    ...
    def test_repository_markdown_includes_untracked_but_not_ignored(self) -> None:
        ...
        subprocess.run(
            ["git", "add", "tracked.md", ".gitignore"], cwd=self.root, check=True
        )
        self.assertEqual(
            {tracked, untracked},
